...
const GoogleMap = () => {
  ...
  const readLocations = async () => {
    try {
      ...
      if (!data) {
        console.log("No Data", "No locations found to read!");
        Speech.speak('No locations found to read Please Provide Destination Details Again',{
          onDone:()=>
            {
            startRecording();
          }
      ...
      Speech.speak('Error reading locations Please Provide Destination Details Again',{
        onDone:()=>
          {
          startRecording();
        }
  ...
  const startRecordingForSavedLocations = async () => {
    try {
      ...
      setTimeout(async () => {
        ...
        // below code will remove the Listening and punctuation marks from recording data
        ...
        if (homechk.trim().toLowerCase().includes("home screen") || homechk.trim().toLowerCase().includes("home  screen")
          || homechk.trim().toLowerCase().includes("homepage") || homechk.trim().toLowerCase().includes("Home page")) {
        ...
        Speech.speak(`You said: ${homechk}`, {
          onDone: () => processTranscript(homechk),
        });
  ...
  const processTranscript = (transcript) => {
    ...
    FileSystem.readAsStringAsync(fileUri).then((data) => {
      ...
      if (chosenLocationIndex > 0 && chosenLocationIndex <= locations.length) {
        const selectedLocation = locations[chosenLocationIndex-1];
        destination=selectedLocation;
        setDestination(destination);
        getDirections();
      } else {
        console.log("Invalid Choice", "The number does not match any location.");
        Speech.speak('Invalid Choice',{
          onDone:()=>
            {
            SavedLocationRead();
          }
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        setTimeout(async () => {
          ...
          if (homechk.trim().toLowerCase().includes("home screen") || homechk.trim().toLowerCase().includes("homescreen")
            || homechk.trim().toLowerCase().includes("homepage") || homechk.trim().toLowerCase().includes("Home page")) {
            
            console.log('Navigating to Home Screen');
            Speech.speak('Navigating to Home Screen',{
              onDone:()=>
                {
                handleBackButton();
              }
        ...
        Speech.speak('Please Confirm The Destination Again ', {
          onDone: ()=>
            {
            confirmMessage();
            return; 
  ...
  const getDirections = async () => {
    ...
    console.log('getDirection >> Provided Destination : '+destination);
    if (!destination) {
